...
검토용 전달본은 `rhwp-android/outputs/rHWP-Fold-<버전>.apk`에 둔다.
...
- 자동복구 사본은 미저장 변경이 있을 때 입력을 2초 쉬면 만들고, 계속 입력 중이면 약 15초 간격과 앱을 떠날 때 저장을 시도한다. 원본 파일의 저장과는 별개다.
- 복구 확인은 앱 시작 때 남은 복구본이 있는 경우 한 번만 표시한다. 편집 중이나 접기·펼치기 때는 표시하지 않는다. 정상 저장 또는 종료 확인에서 **저장 안 함**을 선택하면 해당 문서의 복구본을 지운다.
- 강제 종료 직전의 모든 입력을 보장하지 않고 **마지막으로 완료된 복구본**을 제공한다. 복구한 문서는 수정 상태로 열리므로 저장 위치를 확인해 저장한다.
...
- `Ctrl+1`~`Ctrl+9`, `Ctrl+0`은 현재 문서에 저장된 첫 번째~열 번째 스타일을 적용한다. 이름과 순서는 문서마다 다르다. `Ctrl+0`은 이제 열 번째 스타일이며, 100% 확대는 화면의 확대 도구를 사용한다.
- 한글 마지막 글자가 조합 중인 상태에서도 `Alt+I`, 스타일·서식·저장 단축키와 `F6` 스타일/`F7` 쪽 설정은 조합을 먼저 확정한 뒤 실행한다.
- 본문 상용구에 저장되어 있던 HTML 소스의 들여쓰기 때문에 생기던 여분의 줄바꿈을 제거한다. 사용자가 넣은 실제 문단·줄바꿈은 유지한다. 기존 상용구를 다시 등록할 필요는 없다.
- 기존 웹 편집기는 일부 브라우저 충돌 단축키를 바꿔 사용한다. 예를 들어 오른쪽 정렬은 `Alt+Shift+H`, 가운데 정렬은 `Alt+Shift+C`다. 한컴 한글의 모든 단축키가 동일하다는 뜻은 아니다.

검사 범위와 실기기 확인 상태는 [검증 기록](VALIDATION.md)을 확인한다.

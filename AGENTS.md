# rHWP Fold: 승인된 Android 다운스트림 작업

먼저 `/Users/trayate/projects/AGENTS.md`를 읽는다(해당 로컬 파일이 있는 경우).
사용자가 2026-09-13 승인한 Android 계획은 아래 원본 유지보수 절차보다 우선한다.

- 원본 `cac9b4f7cc743535cd7c00fe4f286abd67e7145b`에서 시작하고, 통합 대상은 내 Fork의 `android-main`이다.
- 사용자는 Fork, 브랜치, commit/push, 내 Fork의 Issue/PR/merge, Android CI와 APK 설치 검증을 승인했다.
- 원본 저장소에는 쓰지 않는다. 상속된 배포/예약 workflow는 사용하지 않는다.
- 의미 있는 작업마다 지금 하는 일, 필요한 개념, 선택 이유/불확실성, 다른 프로젝트에의 적용, 검증 결과를 한국어로 설명한다.
- 추가/수정 코드에는 역할과 이유 중심의 한국어 주석을 쓴다. 외부/생성 코드는 보존한다.
- Android 전용 코드와 문서는 `rhwp-android/`에 둔다. 웹 편집기 변경은 Android 연결 경계에 집중한다.
- Android 변경 검증: Studio 타입 검사/관련 Node 테스트, Android lint/unit test/APK 빌드, 에뮬레이터 및 폴드7 실기기 검사.
- Rust를 바꾸면 아래 원본 Rust 검증 게이트를 따른다. 변경하지 않은 원본 전체의 테스트 성공을 추정하지 않는다.
- 개인 문서, 페어링 코드/기기 주소, 서명키, 토큰은 Git과 공개 로그에 넣지 않는다.
